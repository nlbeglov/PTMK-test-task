...
```bash
java App.class 2 "Ivanov Petr Sergeevich" 2009-07-12 Male
```
...
В 6 пункте необходимо было оптимизировать выборку из таблицы.
К сожалению оптимизацию произвести не удалось. Однако есть простой вариант ускорения работы скрипта - добавить условный LIMIT 100 в конце SQL запроса. В этом случае мы получим ограничиенное количество строк, которое быстрее обрабатывать. =)
